/*
  # La Rue Salon — Full Schema (Salon Only, Safe RLS)

  ## Summary
  Creates all salon tables from scratch for a fresh Supabase project.
  VALM Cosmetics e-commerce is handled entirely by Shopify — no product/order
  tables are created here.

  ## Tables
  1. `profiles`     — Extended auth user data (role: admin | client)
  2. `staff`        — Stylists with schedule info
  3. `services`     — Salon services catalog
  4. `appointments` — Bookings with payment and coupon tracking
  5. `coupons`      — Discount codes
  6. `promotions`   — Sitewide promo rules
  7. `app_settings` — Key-value config store

  ## Security
  - RLS enabled on all tables
  - Uses a SECURITY DEFINER helper `get_current_user_role()` on profiles to
    avoid infinite recursion (admin policies on profiles previously caused the
    "Database error querying schema" crash on login)
  - All admin-check policies use the helper function instead of a
    self-referencing subquery

  ## Admin Seed
  - Seeds admin@larue.mx with role='admin' from auth.users if that user exists
*/
...
-- Helper: reads caller's role bypassing RLS — prevents infinite recursion
...
-- ============================================================
-- SEED ADMIN PROFILE
-- Seeds admin@larue.mx from auth.users if that account exists.
-- ============================================================
